...
export default function AdminPerformance() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const [perfRes, companiesRes] = await Promise.all([
          API.get('/admin/performance'),
          API.get('/admin/companies'),
        ])
        setEmployees(perfRes.data)
  ...
  const filtered = employees.filter((e) => {
    const matchCompany = filterCompany ? e.companyId === parseInt(filterCompany) : true
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Employee performance across all companies</p>
      </div>
              ...
              {filtered.map((emp) => {
                ...
                return (
                  <tr key={emp.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{emp.companyName || '—'}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{emp.role.replace('_', ' ')}</td>
