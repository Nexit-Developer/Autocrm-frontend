...
export default function Payroll() {
    ...
    const getData = async () => {
      try {
        const [empRes, payrollRes] = await Promise.all([
          ...
          API.get(`/hr/payroll?month=${selectedMonth}`),
        ])
  ...
  const refreshPayrolls = async () => {
    try {
      const res = await API.get(`/hr/payroll?month=${selectedMonth}`)
    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage monthly employee payroll</p>
        </div>
        <input
          type="month"
          value={selectedMonth}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left text-xs font-medium text-gray-500 px-4 py-3">Basic salary</th>
                ...
                <th className="text-left text-xs font-medium text-gray-500 px-4 py-3">Net salary</th>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {payrollModal.name} — {selectedMonth}
            </p>
